...
Integration note (branch phase7-integration):
    Inputs are produced by src/llm/risk_adapter.py from the validated
    Phase 5 output (data/processed/risk_scores.csv):

        delay_prediction_days = Phase 5 predicted_delay_days (renamed)
        computed_critical     = Phase 5 computed_critical (preserved;
                                is_critical added as an identical alias)
        risk_score            = Phase 5 risk_score (carried through unchanged)

    risk_level is a deterministic classification of the Phase 5
    risk_score using the SAME thresholds this analyzer applies
    (High >= 0.70, Medium >= 0.40). It is NOT a second risk-scoring
    system, and the underlying score is never altered.
"""
...
class ConstructionRiskAnalyzer:
    ...
    def analyze_activity(
        ...
        # Integration note (branch phase7-integration): the Phase 7 adapter
        # (src/llm/risk_adapter.py) supplies risk_level already, classified
        # with the SAME thresholds as _risk_level below. Reuse it when
        # present (identical outcome, single source of truth for the
        # thresholds), otherwise classify deterministically here.
        provided_level = row.get("risk_level")
        if isinstance(provided_level, str) and provided_level in {
            "High",
            "Medium",
            "Low",
        }:
